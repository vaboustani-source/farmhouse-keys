CREATE OR REPLACE FUNCTION public.lookup_guest_booking(p_email text, p_event_slug text, p_section_slug text)
 RETURNS TABLE(booking_id uuid, event_id uuid, section_id uuid, guest_name text, guest_email text, payment_status text, payment_schedule text, deposit_paid_at timestamp with time zone, final_paid_at timestamp with time zone, covered_at timestamp with time zone, total_amount numeric, base_amount numeric, addon_amount numeric, resort_fee numeric, tax_amount numeric, addons_selected jsonb, is_primary boolean, cot_requested boolean, cot_fee numeric, wedding_name text, couple_names text, wedding_date date, check_in_date date, check_out_date date, section_name text, guest_nightly_rate numeric, resort_fee_percent numeric, nights integer, booking_link_slug text, cot_1night_rate numeric, cot_2night_rate numeric)
 LANGUAGE sql
